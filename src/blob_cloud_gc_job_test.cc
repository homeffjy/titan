...
class CloudGCJobTest : public testing::Test {
 public:
  std::string dbname_;
  TitanDB* db_;
  DBImpl* base_db_{};
...
TEST_F(CloudGCJobTest, DiscardEntry) { TestDiscardEntry(); }

TEST_F(CloudGCJobTest, RunGC) { TestRunGC(); }

TEST_F(CloudGCJobTest, GCLimiter) {
...
// snapshots.
TEST_F(CloudGCJobTest, PurgeBlobs) {
  NewDB();
...
TEST_F(CloudGCJobTest, GCWhileDeleteFilesInRange) {
  NewDB();
  ...
  rocksdb::SyncPoint::GetInstance()->LoadDependency({
      {"BlobGCJob::Finish::BeforeDeleteInputBlobFiles",
       "BlobCloudGCJobTest::GCWhileDeleteFilesInRange:Wait"},
      {"BlobFileSet::LogAndApply::Wait",
       "BlobCloudGCJobTest::GCWhileDeleteFilesInRange:1"},
      {"BlobCloudGCJobTest::GCWhileDeleteFilesInRange:2",
  ...
  rocksdb::port::Thread t2([&]() {
    TEST_SYNC_POINT("BlobCloudGCJobTest::GCWhileDeleteFilesInRange:Wait");
  ...
  // `LogAndApply`
  TEST_SYNC_POINT("BlobCloudGCJobTest::GCWhileDeleteFilesInRange:1");
  std::this_thread::sleep_for(std::chrono::seconds(1));
  TEST_SYNC_POINT("BlobCloudGCJobTest::GCWhileDeleteFilesInRange:2");
  t1.join();
  t2.join();
...
TEST_F(CloudGCJobTest, DeleteFilesInRange) {
  NewDB();
...
TEST_F(CloudGCJobTest, LevelMergeGC) {
...
TEST_F(CloudGCJobTest, RangeMergeScheduler) {
  NewDB();
  auto init_files =
...
TEST_F(CloudGCJobTest, RangeMerge) {
  options_.level_merge = true;
  options_.level_compaction_dynamic_level_bytes = true;
  options_.blob_file_discardable_ratio = 0.5;
